...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "db_host": r"\b[\w.-]+\.(?:internal|local)\b(?::\d+)?",
...
async def llm_safety_check(response_text: str) -> dict:
    ...
    try:
        ...
        # A transient judge failure must not take down the whole pipeline;
        # content_filter() already ran as a deterministic first line of defense.
        return {"safe": True, "verdict": f"Judge error — skipped ({e})"}
            ...
            if not judge_result["safe"]:
                ...
                llm_response.content = types.Content(
                    ...
                    parts=[types.Part.from_text(
                        text="Xin lỗi, phản hồi không đáp ứng tiêu chuẩn an toàn "
                             "và đã bị chặn."
                    )],
                )
